...
public class AppointmentApprovalForm extends javax.swing.JFrame {
    ...
    public AppointmentApprovalForm() {
        initComponents();
        setLocationRelativeTo(null);
        setupTable();
    }

    public AppointmentApprovalForm(User currentUser) {
        this();
    ...
    private void loadAppointments() {
        ...
        DefaultTableModel m = (DefaultTableModel) tblAppointments.getModel();
        m.setRowCount(0);
        for (Appointment a : new AppointmentDAO().getAppointmentsForDentist(currentUser.getDentistId())) {
            m.addRow(new Object[]{a.getAppointmentNo(), a.getPatientName(), a.getTreatmentType()
    ...
    private void updateStatus(String status) {
        ...
        String no = tblAppointments.getValueAt(row, 0).toString(), current = tblAppointments.getValueAt(row, 5).toString();
        if (!"PENDING".equalsIgnoreCase(current)) {
            JOptionPane.showMessageDialog(this, "Only pending appointments can be approved or declined.");
            return;
        }
        
        if (JOptionPane.showConfirmDialog(this, "Change appointment status to " + status + "?", "Confirm", 
                JOptionPane.YES_NO_OPTION) != JOptionPane.YES_OPTION) {
            return;
        }
        
        if (new AppointmentDAO().updateAppointmentStatus(no, currentUser.getDentistId(), status)) {
            JOptionPane.showMessageDialog(this, "Appointment status updated to " + status + ".");
            loadAppointments();
        } 
        
        else {
            JOptionPane.showMessageDialog(this, "Unable to update appointment.");
        }
    }
